...
def info(protease):
    ...
    if protease == 'Trypsin':
        ...
        print("Error: this tool does not support %s protease,"
              " only 'Trypsin', 'ArgC', 'Chymotrypsin', 'GluC',"
              " 'LysC', 'AspN', 'LysN' and 'LysargiNase' are"
              " optional."%protease)
        sys.exit(1)
    return sites, ter
...
#    np.savetxt("protein_sequences.txt", s, fmt="%s", 
#                delimiter='\t', newline='\n', comments='')
#    my_matrix = np.loadtxt(open("protein_sequences.txt", "rb"), 
#                            dtype=str, delimiter=" ", skiprows=0)
    e1 = time.time()
    ...
    for protein in s:
        ...
        peptide_list = cut_and_gene_tradata.digestion(proseq, sites, ter, 
                                                      maxlen, minlen, 
                                                      missedcleavages)
        # peptide_list = digestion(proseq, sites, ter, maxlen, minlen, 
        #                          missedcleavages)
...
if __name__ == '__main__':
    ...
    AI_tool(datapath, resultpath, regular, protease, 
            maxlen, minlen, missedcleavages)
    #os.remove('protein_sequences.txt')
