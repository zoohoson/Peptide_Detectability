...
def build_model():
    ...
    net_merge = tf.keras.layers.Dense(32, activation='relu')(merge)
    # net_merge = tf.keras.layers.Dropout(np.random.uniform(0, 0.2))(net_merge)
...
def main():
    ...
    np.save('log/DbyDeep_log_other.npy', history.history)
    plt.figure(figsize=(16,2))
    plt.subplot(1,2,1)
    plot_graphs(history, 'accuracy')
    plt.subplot(1,2,2)
    plot_graphs(history, 'loss')
    plt.savefig('log/DbyDeep_other.png', bbox_inches='tight')
    
    model.save("log/DbyDeep_other.h5")
