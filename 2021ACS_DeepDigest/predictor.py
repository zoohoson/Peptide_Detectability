...
def predictor(data, resultpath, protease):
    ...
    for line in data:
        ...
        if len(array) == 4:
            # left 31-mers' cleavage probability
            if array[1] != '*':
                array[1] = float(1 - proba[i])
                i += 1
            else:
                array[1] = float(1)
            # right 31-mers' cleavage probability
            if array[2] != '*':
            ...
            # mispro --> missing 31-mers' cleavage probability (maybe multiple)
            # k --> missed cleavage probability (one)
    ...
    with open(resultpath, 'w') as r:
        header = 'Protein id\tPeptide sequence\t' + \
                    'Digestibility of N-terminal site\t' + \
                    'Digestibility of C-terminal site\t' + \
                    'Digestibility of the missed site\n'
        r.write(header)
        for line in results:
            r.write('\t'.join(line) + '\n')
